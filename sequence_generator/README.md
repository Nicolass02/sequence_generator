...
## Examples
An tmap (tmap.html) example can be found in the folder "tmaps". It was made from the text file "tmap.txt", which contains various sequences of differing combination lengths as well as variation between linear and cyclic sequences (as long as the sequences are the same ones, putting them in a specific order will not affect the result). The text file contains roughly 33'000 sequences. Another example made with a bit less than 330'000 sequences can be found under the following link: [link](link)